"""Export a completed Round 1.6 run using a strict scalar-only contract."""
...
DEFAULT_DESTINATION = REPO_ROOT / "results" / "camera_head_amplification"
...
def export_numeric_results(
    ...
    if forbidden:
        raise ValueError(f"forbidden artifact in Round 1.6 run: {forbidden[0]}")
    ...
    if metadata.get("study_name") != "camera_head_amplification":
        raise ValueError("source is not a Camera Head amplification run")
    if complete.get("run_id") != run_id:
        raise ValueError("complete.json run_id mismatch")
    if complete.get("baseline_checks_passed") is not True:
        raise ValueError("baseline replay checks did not pass")

    candidates: list[Path] = []
    for filename in FILES:
        path = source / filename
        if not path.is_file() or path.stat().st_size <= 0:
            raise FileNotFoundError(f"required numeric result is missing or empty: {path}")
